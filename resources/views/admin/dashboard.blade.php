                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center justify-center gap-2">
                                ...
                                <form method="POST" action="{{ route('admin.projects.destroy', $project) }}" class="inline delete-form-{{ $project->id }}">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" onclick="deleteProject({{ $project->id }}, document.querySelector('.delete-form-{{ $project->id }}'))" class="p-2 rounded-lg hover:bg-rose-500/10 text-rose-400 hover:text-rose-300 transition-all group" title="Hapus">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
