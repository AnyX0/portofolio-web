    ...
    <div class="w-full max-w-md rounded-3xl border border-white/10 bg-white/5 p-8 shadow-2xl shadow-cyan-500/10">
        ...
        <form method="POST" action="{{ route('admin.login.store') }}" class="space-y-4">
            ...
            <div class="space-y-2">
                ...
                <input name="email" type="email" value="{{ old('email') }}" class="w-full rounded-xl bg-slate-900/80 border border-white/10 px-4 py-3 text-sm focus:border-cyan-400 focus:outline-none" required autofocus>
            </div>
            <div class="space-y-2">
