...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.layouts.app','data' => ['title' => 'Daftar Project']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['title' => 'Daftar Project']); ?>
    <div class="flex items-center justify-between mb-6">
        ...
        <a href="<?php echo e(route('admin.projects.create')); ?>" class="inline-flex items-center gap-2 px-4 py-3 rounded-lg bg-gradient-to-r from-cyan-400 to-indigo-500 text-slate-950 font-semibold text-sm shadow-lg shadow-indigo-500/20 hover:shadow-indigo-500/40 transition">
    ...
    <div class="grid md:grid-cols-4 gap-3 mb-6">
        <div class="rounded-lg border border-white/10 bg-white/5 p-3">
            ...
            <p class="text-xl font-semibold text-white"><?php echo e($totalCount); ?></p>
        </div>
        <div class="rounded-lg border border-emerald-400/20 bg-emerald-500/10 p-3">
            <p class="text-xs text-slate-400 mb-1">Published</p>
            <p class="text-xl font-semibold text-emerald-300"><?php echo e($publishedCount); ?></p>
        </div>
        <div class="rounded-lg border border-amber-400/20 bg-amber-500/10 p-3">
            <p class="text-xs text-slate-400 mb-1">Draft</p>
            <p class="text-xl font-semibold text-amber-200"><?php echo e($draftCount); ?></p>
        </div>
        <div class="rounded-lg border border-white/10 bg-white/5 p-3">
            <p class="text-xs text-slate-400 mb-1">Recent</p>
            <p class="text-xl font-semibold text-cyan-300"><?php echo e($recentCount); ?></p>
        </div>
    </div>
    ...
    <div class="mb-4 flex items-center justify-between">
        <div class="flex gap-2">
            <a href="<?php echo e(route('admin.projects.index')); ?>" class="px-3 py-2 rounded-lg text-sm <?php echo e(!request('status') ? 'bg-white/10 text-white' : 'text-slate-400 hover:bg-white/5'); ?>">Semua</a>
            <a href="<?php echo e(route('admin.projects.index', ['status' => 'published'])); ?>" class="px-3 py-2 rounded-lg text-sm <?php echo e(request('status') === 'published' ? 'bg-emerald-500/20 text-emerald-300' : 'text-slate-400 hover:bg-white/5'); ?>">Published</a>
            <a href="<?php echo e(route('admin.projects.index', ['status' => 'draft'])); ?>" class="px-3 py-2 rounded-lg text-sm <?php echo e(request('status') === 'draft' ? 'bg-amber-500/20 text-amber-300' : 'text-slate-400 hover:bg-white/5'); ?>">Draft</a>
        </div>
        <a href="<?php echo e(route('admin.projects.archived')); ?>" class="inline-flex items-center gap-2 px-3 py-2 rounded-lg text-sm text-slate-400 hover:text-white hover:bg-white/5 transition">
                ...
                <thead class="bg-white/5 sticky top-0">
                    <tr>
                        <th class="text-left px-4 py-3">
                            <a href="<?php echo e(route('admin.projects.index', array_merge(request()->query(), ['sort' => 'title']))); ?>" class="hover:text-white">Judul</a>
                        </th>
                        <th class="text-left px-4 py-3">Tech Stack</th>
                        <th class="text-center px-4 py-3">Status</th>
                        <th class="text-center px-4 py-3">
                            <a href="<?php echo e(route('admin.projects.index', array_merge(request()->query(), ['sort' => 'published_at']))); ?>" class="hover:text-white">Dipublikasikan</a>
                        </th>
                ...
                <?php $__empty_1 = true; $__currentLoopData = $projects; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $project): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <tr class="border-t border-white/5 hover:bg-white/5 transition">
                        <td class="px-4 py-3 font-medium text-white max-w-xs">
                            <a href="<?php echo e(route('admin.projects.edit', $project)); ?>" class="hover:text-cyan-300 truncate block"><?php echo e($project->title); ?></a>
                        </td>
                            ...
                            <?php if($project->tech_stack): ?>
                                <div class="flex flex-wrap gap-1">
                                    <?php $__currentLoopData = explode(',', $project->tech_stack); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $tech): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <span class="px-2 py-1 rounded bg-indigo-500/20 text-indigo-300"><?php echo e(trim($tech)); ?></span>
                        ...
                        <td class="px-4 py-3 text-center text-sm">
                            <?php echo e(optional($project->published_at)->format('d M Y') ?? '-'); ?>
                        ...
                        <td class="px-4 py-3 text-center">
                            <div class="inline-flex items-center justify-center gap-2 bg-white/5 rounded-lg p-1">
                                <!-- Edit -->
                                <a href="<?php echo e(route('admin.projects.edit', $project)); ?>" title="Edit" class="inline-flex items-center justify-center w-8 h-8 rounded text-amber-400 hover:bg-amber-500/20 transition">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                </a>
                                
                                <!-- Archive -->
                                <button type="button" onclick="archiveProject('<?php echo e($project->id); ?>')" title="Arsipkan" class="inline-flex items-center justify-center w-8 h-8 rounded text-blue-400 hover:bg-blue-500/20 transition">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8v10a2 2 0 002 2h10a2 2 0 002-2V8m-9 4h4"/></svg>
                                </button>
                                
                                <!-- Share -->
                                <button type="button" onclick="shareProject('<?php echo e($project->id); ?>', '<?php echo e($project->title); ?>')" title="Bagikan" class="inline-flex items-center justify-center w-8 h-8 rounded text-indigo-400 hover:bg-indigo-500/20 transition">
                                ...
                                <!-- Delete -->
                                <form method="POST" action="<?php echo e(route('admin.projects.destroy', $project)); ?>" class="inline deleteForm-<?php echo e($project->id); ?>">
                                    <?php echo csrf_field(); ?>
                                    <?php echo method_field('DELETE'); ?>
                                    <button type="button" title="Hapus" onclick="deleteProject('<?php echo e($project->id); ?>', document.querySelector('.deleteForm-<?php echo e($project->id); ?>'))" class="inline-flex items-center justify-center w-8 h-8 rounded text-rose-400 hover:bg-rose-500/20 transition">
                ...
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                    <tr>
                        <td colspan="6" class="px-4 py-8 text-center text-slate-400">
                            ...
                            Belum ada project<?php echo e(request('status') ? ' dengan status ini' : ''); ?>.
                        </td>
                    </tr>
                <?php endif; ?>
                </tbody>
    ...
    <div class="mt-6 flex items-center gap-2 text-sm text-slate-400">
        ...
        Menampilkan <?php echo e($projects->count()); ?> dari <?php echo e($projects->total()); ?> project
    </div>
    ...
    <script>
        ...
        function shareProject(projectId, projectTitle) {
            currentProjectId = projectId;
            const projectUrl = `<?php echo e(url('/project')); ?>/${projectId}`;
...
<?php endif; ?>
<?php /**PATH E:\Andi\Documents\Kuliah\PEMROGRAMAN MOBILE - SMT5\StudioProjects\project_uas\resources\views/admin/projects/index.blade.php ENDPATH**/ ?>