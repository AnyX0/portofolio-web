...
<div class="space-y-2">
    ...
    <input name="title" type="text" value="<?php echo e(old('title', optional($current)->title)); ?>" class="w-full rounded-xl bg-slate-900/70 border border-white/10 px-4 py-3 text-sm focus:border-cyan-400 focus:outline-none" required>
    <?php $__errorArgs = ['title'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?><p class="text-rose-200 text-xs"><?php echo e($message); ?></p><?php unset($message);
...
<div class="space-y-2">
    ...
    <textarea name="summary" class="w-full rounded-xl bg-slate-900/70 border border-white/10 px-4 py-3 text-sm focus:border-cyan-400 focus:outline-none" rows="3" required><?php echo e(old('summary', optional($current)->summary)); ?></textarea>
...
<div class="space-y-2">
    ...
    <textarea name="description" class="w-full rounded-xl bg-slate-900/70 border border-white/10 px-4 py-3 text-sm focus:border-cyan-400 focus:outline-none" rows="6"><?php echo e(old('description', optional($current)->description)); ?></textarea>
...
<div class="grid md:grid-cols-2 gap-4">
    <div class="space-y-2">
        ...
        <input name="tech_stack" type="text" value="<?php echo e(old('tech_stack', optional($current)->tech_stack)); ?>" class="w-full rounded-xl bg-slate-900/70 border border-white/10 px-4 py-3 text-sm focus:border-cyan-400 focus:outline-none">
        <?php $__errorArgs = ['tech_stack'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?><p class="text-rose-200 text-xs"><?php echo e($message); ?></p><?php unset($message);
...
unset($__errorArgs, $__bag); ?>
    ...
    <div class="space-y-2">
        ...
        <input name="live_url" type="url" value="<?php echo e(old('live_url', optional($current)->live_url)); ?>" class="w-full rounded-xl bg-slate-900/70 border border-white/10 px-4 py-3 text-sm focus:border-cyan-400 focus:outline-none">
        <?php $__errorArgs = ['live_url'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?><p class="text-rose-200 text-xs"><?php echo e($message); ?></p><?php unset($message);
...
<div class="space-y-2">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-3" id="existingImages">
        <?php $__currentLoopData = $images; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $image): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            ...
            <div class="relative group" data-image-url="<?php echo e($image); ?>" data-public-id="<?php echo e($publicId); ?>">
                <img src="<?php echo e($image); ?>" alt="Existing" class="w-full h-24 object-cover rounded-lg border border-white/10 group-hover:border-cyan-400 transition">
                <button 
                    type="button"
                    ...
                    data-public-id="<?php echo e($publicId); ?>">
                    ×
                </button>
...
<div class="space-y-2">
    ...
    <input name="repo_url" type="url" value="<?php echo e(old('repo_url', optional($current)->repo_url)); ?>" class="w-full rounded-xl bg-slate-900/70 border border-white/10 px-4 py-3 text-sm focus:border-cyan-400 focus:outline-none">
    <?php $__errorArgs = ['repo_url'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?><p class="text-rose-200 text-xs"><?php echo e($message); ?></p><?php unset($message);
...
<label class="inline-flex items-center gap-2 text-sm text-slate-300">
    ...
    <input type="checkbox" name="is_published" value="1" <?php echo e(old('is_published', optional($current)->is_published ?? true) ? 'checked' : ''); ?> class="rounded border-white/20 bg-slate-900/80">
    Publish segera
</label>
...
<div class="flex items-center gap-3 pt-2">
    ...
    <a href="<?php echo e(route('admin.dashboard')); ?>" class="text-sm text-slate-300 hover:text-white">Batal</a>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (form) {
        form.addEventListener('submit', async function(e) {
            ...
            try {
                const cloudinaryFolder = '<?php echo e($project->cloudinary_folder ?? "portfolio/projects/temp"); ?>';
    ...
    async function uploadToCloudinary(file, folder) {
        ...
        const response = await fetch('<?php echo e(route("admin.uploadImage")); ?>', {
            method: 'POST',
            headers: {
    ...
    document.querySelectorAll('.deleteImageBtn').forEach(btn => {
        btn.addEventListener('click', function(e) {
            ...
            fetch('<?php echo e(isset($project) ? route("admin.projects.deleteImage", $project) : "#"); ?>', {
                method: 'POST',
                headers: {
...
<?php /**PATH E:\Andi\Documents\Kuliah\PEMROGRAMAN MOBILE - SMT5\StudioProjects\portofolio-web\resources\views/admin/projects/partials/form.blade.php ENDPATH**/ ?>